...
void ChapterBackgroundManager::LoadBackgroundSet(int chapter)
{
	...
	if (chapter == 3 || chapter == 4)
	{
		...
		CreateLayerSet(m_Tiles, tileTextures, 0.0f, -1);			  // 타일
		CreateLayerSet(m_TileEdges, tileEdgesTextures, 0.0f, -2);     // 타일 테두리
		CreateLayerSet(m_Backgrounds, backgroundTextures, 0.0f, -7);  // 배경
		CreateLayerSet(m_ForeLayers, foreTextures, 80.0f, -8);		  // 전경
		CreateLayerSet(m_FarLayers, farTextures, 20.0f, -9);		  // 원경
