...
// #pragma endregion
...

	{
		...
		gameObject->m_Name = "test";
		auto trans = gameObject->GetComponent<TransformComponent>();
		trans->SetPosition({ -800.0f, 500.0f });
		auto sr = gameObject->AddComponent<SpriteRenderer>();
		sr->SetAssetManager(&m_AssetManager);
		auto& clips = m_AssetManager.LoadAnimation(L"SourRun", L"../Resource/Character/SourRun.json");
		...
		sr->SetPath("../Resource/SourRun.json");
		sr->SetTextureKey("SourRun");
...
#pragma endregion
  ...
  /*sr->SetTexture(bitmap);
	...
	//m_UIManager.AddUI("TestScene", soundUI);
	//m_UIManager.AddUI(buttonUI);
	//m_UIManager.AddUI("TestScene", uiObject);
